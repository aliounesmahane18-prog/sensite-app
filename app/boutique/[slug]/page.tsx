...
export default function CataloguePage() {
  ...
  const handleAddToCart = (product: Product, variants: Record<string, string> = {}) => {
    ...
    setCart(prev => {
      ...
      if (existing) {
        return prev.map(i => `${i.product.id}-${JSON.stringify(i.selectedVariants)}` === key
  ...
  if (notFound || !boutique) return (
    <div className="min-h-screen flex items-center justify-center text-center px-4">
      <div><p className="text-5xl mb-4">🏪</p><h1 className="text-2xl font-bold mb-2">Boutique introuvable</h1></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {boutique.logo_url ? (
              ...
              <div className="w-11 h-11 rounded-xl bg-white/20 border-2 border-white/30 flex items-center justify-center font-bold text-sm">
                {boutique.name.slice(0, 2).toUpperCase()}
              </div>
            )}
            <div>
              <h1 className="font-bold text-lg leading-tight">{boutique.name}</h1>
            ...
            {products.map(p => (
              <div key={p.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="h-40 bg-gray-100 relative">
                  ...
                  {p.is_featured && <span className="absolute top-2 left-2 bg-yellow-400 text-yellow-900 text-xs font-bold px-2 py-0.5 rounded-full">⭐</span>}
                  {p.has_variants && <span className="absolute top-2 right-2 bg-white/90 text-gray-600 text-xs font-semibold px-2 py-0.5 rounded-full">Variantes</span>}
                </div>
                <div className="p-3">
                  <p className="font-semibold text-sm text-gray-900 line-clamp-2">{p.name}</p>
                  <div className="flex items-end justify-between mt-2">
                    <div>
                      <p className="font-bold text-sm" style={{ color }}>{fmt(p.price)} FCFA</p>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-end">
          <div className="flex-1 bg-black/40" onClick={() => setSelectedProduct(null)} />
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl p-5 space-y-4 max-h-[80vh] overflow-y-auto">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-bold" style={{ color }}>{fmt(selectedProduct.price)} FCFA</p>
              </div>
            </div>
                    ...
                    <button key={i} onClick={() => setSelectedVariants(prev => ({ ...prev, [variant.name]: val }))}
                      className={`px-3 py-1.5 rounded-xl border text-sm font-medium transition-all ${
                        selectedVariants[variant.name] === val
                          ? "border-2 text-white" : "border-gray-200 text-gray-700 bg-white"
                      }`}
                      style={selectedVariants[variant.name] === val ? { background: color, borderColor: color } : {}}>
                      {val}
                    </button>
                  ))}
                </div>
          ...
          <div className="bg-white rounded-t-3xl max-h-[85vh] overflow-hidden flex flex-col">
                  ...
                  {cart.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-bold" style={{ color }}>{fmt(item.product.price * item.quantity)} FCFA</p>
                      </div>
                      <div className="flex items-center gap-1">
                        <button onClick={() => setCart(prev => {
                          const updated = prev.map((i, j) => j === idx ? { ...i, quantity: i.quantity - 1 } : i);
                          return updated.filter(i => i.quantity > 0);
                        })} className="w-7 h-7 bg-gray-100 rounded-lg text-center text-lg">−</button>
                        <span className="w-6 text-center text-sm font-bold">{item.quantity}</span>
                        <button onClick={() => setCart(prev => prev.map((i, j) => j === idx ? { ...i, quantity: i.quantity + 1 } : i))}
                          className="w-7 h-7 rounded-lg text-center text-white text-lg" style={{ background: color }}>+</button>
                      </div>
                    </div>
                  ))}
                  <div className="pt-3 border-t border-gray-100 flex justify-between font-bold">
                    <span>Total</span><span style={{ color }}>{fmt(total)} FCFA</span>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  <div><label className="text-sm font-medium text-gray-700 mb-1 block">Ton nom *</label>
                    <input type="text" value={orderForm.name} onChange={e => setOrderForm({ ...orderForm, name: e.target.value })}
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-300" placeholder="Fatou Diallo" /></div>
                  <div><label className="text-sm font-medium text-gray-700 mb-1 block">Ton WhatsApp *</label>
                    <input type="tel" value={orderForm.phone} onChange={e => setOrderForm({ ...orderForm, phone: e.target.value })}
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-300" placeholder="77 123 45 67" /></div>
                  <div><label className="text-sm font-medium text-gray-700 mb-1 block">Quartier / Adresse</label>
                    <input type="text" value={orderForm.address} onChange={e => setOrderForm({ ...orderForm, address: e.target.value })}
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-300" placeholder="Médina, Dakar..." /></div>
            ...
            {cart.length > 0 && (
              <div className="p-4 border-t border-gray-100">
                {step === "cart" ? (
                  <button onClick={() => setStep("form")} className="w-full py-3.5 rounded-2xl text-white font-bold" style={{ background: color }}>
                    Commander →
                  </button>
                ) : (
                  <div className="space-y-2">
                    <button onClick={() => setStep("cart")} className="w-full py-3 rounded-2xl border border-gray-200 font-semibold text-gray-700">← Retour</button>
                    <button onClick={sendOrder} className="w-full py-3.5 rounded-2xl bg-green-500 text-white font-bold">
